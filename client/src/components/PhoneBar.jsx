...
import { Phone, PhoneOff, Mic, MicOff, Grid3X3, Activity, Zap, Clock } from 'lucide-react';
...
export default function PhoneBar({
  ...
  const countingDown = countdownProgress !== null;
  ...
  function getStatusText() {
    ...
    if (countingDown) return 'Auto-dialing...';
    ...
    <div className="bg-white border-b shadow-sm">
      <div className="flex items-center h-14 px-4 gap-4">
        <div className="flex flex-col min-w-[200px]">
          ...
          <div className="h-1 mt-1 w-full bg-gray-100 rounded-full overflow-hidden">
            {countingDown && (
              <div
                className="h-full bg-calljet-500 transition-none"
        ...
        {isIdle ? (
          <button onClick={onDial} disabled={!canDial} className="btn-call" title="Call (or press Escape to cancel auto-dial)">
            <Phone className="w-5 h-5" />
          </button>
        ) : (
          <button onClick={onHangup} className="btn-hangup" title="Hang up">
        ...
        <button
          onClick={toggleMute}
          disabled={!isActive}
          className={`p-2.5 rounded-full transition-colors ${isMuted ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'} disabled:opacity-30`}
        ...
        {isProgressive && (
          <div className="flex items-center gap-1 border-l pl-3">
            <button
              ...
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors ${autoDialEnabled ? 'bg-calljet-100 text-calljet-700 hover:bg-calljet-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
            ...
            <button
              onClick={() => setAutoDialDelayEnabled(!autoDialDelayEnabled)}
              disabled={!autoDialEnabled}
              className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium transition-colors disabled:opacity-40 ${autoDialDelayEnabled ? 'bg-amber-100 text-amber-700 hover:bg-amber-200' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
              title="3-second delay before each auto-dial"
            >
              <Clock className="w-3.5 h-3.5" />
              3s delay {autoDialDelayEnabled ? 'ON' : 'OFF'}
            </button>
          </div>
        )}
        ...
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-medium flex items-center gap-2 transition-colors ${showActivity ? 'bg-calljet-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
        >
