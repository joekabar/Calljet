...
import { X, Phone, FileText, Clock, CheckCircle, MessageSquare } from 'lucide-react';
...
export default function ActivityPanel({ lead, onClose }) {
  ...
  function getActionIcon(a) { return a==='call'?<Phone className="w-3.5 h-3.5"/>:a==='lead_saved'?<CheckCircle className="w-3.5 h-3.5"/>:a==='callback_set'?<Clock className="w-3.5 h-3.5"/>:a==='note'?<FileText className="w-3.5 h-3.5"/>:<MessageSquare className="w-3.5 h-3.5"/>; }
  function getActionColor(a) { return a==='call'?'bg-blue-100 text-blue-600':a==='lead_saved'?'bg-green-100 text-green-600':a==='callback_set'?'bg-amber-100 text-amber-600':'bg-gray-100 text-gray-600'; }
  function fmtStatus(s) { const l={success:'Success',not_interested:'Not interested',unqualified:'Unqualified',invalid:'Invalid',auto_redial:'Automatic callback',private_callback:'Private callback',shared_callback:'Shared callback',vip_callback:'VIP callback'}; return l[s]||s; }
