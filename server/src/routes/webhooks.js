...
import { sendToUser, broadcastToAll } from '../services/websocket.js';

const router = Router();

// Telnyx call webhooks
router.post('/telnyx/call', async (req, res) => {
  try {
    ...
    switch (eventType) {
      ...
      case 'call.answered': {
        // Call was answered - update call record
        if (telnyxCallId) {
          await supabase
            .from('calls')
            .update({
      ...
      case 'call.hangup': {
        ...
        else if (cause === 'NORMAL_TEMPORARY_FAILURE') disposition = 'congestion';
    ...
    res.status(200).send('OK'); // Always return 200 to Telnyx
  }
});
