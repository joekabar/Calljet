...
import { Upload, Search, ChevronLeft, ChevronRight, Phone } from 'lucide-react';
...
export default function Leads() {
  ...
  const statusOptions = [
    ...
    { value: 'auto_redial', label: 'Auto redial' },
    { value: 'private_callback', label: 'Private callback' },
    { value: 'shared_callback', label: 'Shared callback' },
  ...
  function getStatusColor(status) {
    const colors = {
      ...
      auto_redial: 'bg-blue-100 text-blue-700',
      private_callback: 'bg-indigo-100 text-indigo-700',
      shared_callback: 'bg-purple-100 text-purple-700',
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500">{campaign?.name} · {total} total leads</p>
        </div>
        <div>
        ...
        <form onSubmit={handleSearch} className="flex-1 flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search by phone or company..."
            />
          </div>
              ...
              leads.map(lead => (
                <tr key={lead.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-400 text-xs">
                    {lead.last_contacted_at ? new Date(lead.last_contacted_at).toLocaleString('nl-BE', { dateStyle: 'short', timeStyle: 'short' }) : '-'}
                  </td>
                </tr>
              ))
            )}
