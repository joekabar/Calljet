...
export default function LeadPanel({ lead, campaign, leadData, onUpdateField, onFetchNext, fetchingLead }) {
      ...
      <div className="flex flex-col items-center justify-center h-full text-gray-400">
        {fetchingLead ? (
          ...
          <>
            ...
            <p className="font-medium">No active lead</p>
            <button onClick={onFetchNext} className="btn-primary mt-4 text-sm">
              Get next lead
            </button>
          </>
        )}
  ...
  const fields = campaign?.campaign_lead_fields?.length > 0
    ? campaign.campaign_lead_fields
        .filter(f => f.active)
        .sort((a, b) => a.field_order - b.field_order)
    : Object.keys(leadData).map((key, i) => ({
        field_name: key,
        field_type: key.toLowerCase().includes('phone') || key.toLowerCase().includes('tel') ? 'phone' : 'text',
        field_order: i,
        editable: true
      }));
  ...
  function getStatusBadge(status) {
    const colors = {
      ...
      vip_callback: 'bg-amber-100 text-amber-700',
    ...
    <div>
      ...
      <div className="mb-6">
        <div className="flex items-center gap-3 mb-2">
          <span className="text-xs text-gray-400 font-medium">Campaign</span>
