...
export function AuthProvider({ children }) {
  ...
  async function loadProfile() {
    try {
      ...
      if (err.status === 403) {
        setAccessDeniedError('Your account is not approved. Contact your administrator.');
  ...
  return <AuthContext.Provider value={{ user, profile, loading, accessDeniedError, signIn, signOut, loadProfile }}>{children}</AuthContext.Provider>;
}
